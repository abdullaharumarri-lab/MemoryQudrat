...
def add_or_reset_weak_question(quiz_id: int, question_id: int):
    ...
    cursor.execute(
        """INSERT INTO weak_questions (quiz_id, question_id, stage, next_review_date)
           ...
           DO UPDATE SET stage = 0, next_review_date = ?""",
        (quiz_id, question_id, today, today),
    )
    conn.commit()
    conn.close()
...
def get_session() -> dict | None:
    ...
    if row:
        ...
        d["wrong_ids"] = json.loads(d["wrong_ids"])
        return d
    return None
...
def set_last_message_id(chat_id: int, message_id: int):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO bot_state (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = ?",
        (f"last_msg_{chat_id}", str(message_id), str(message_id))
    )
    conn.commit()
    conn.close()
